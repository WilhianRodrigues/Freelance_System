...
@section('title', 'Avaliar Freelancer')

@section('content')
    <div class="container px-4 py-8 mx-auto">
        <h1 class="mb-6 text-2xl font-bold">Avaliar Freelancer</h1>

        <div class="p-6 bg-white rounded-lg shadow">
            <h2 class="mb-4 text-xl font-semibold">Projeto: {{ $project->title }}</h2>
            <p class="mb-6">Você está avaliando: <span class="font-medium">{{ $user->name }}</span></p>

            <form method="POST" action="{{ route('cliente.ratings.store_freelancer', $project) }}">
                @csrf
